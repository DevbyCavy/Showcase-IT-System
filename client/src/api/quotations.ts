...
// Opens the quotation as a standalone HTML page in a new tab so the user can print it (Ctrl+P ->
// Save as PDF) via the browser's own renderer, bypassing server-side Puppeteer PDF generation
// entirely. window.open must be called synchronously (before the first await) so browsers still
// attribute it to the click that triggered this, or popup blockers will swallow it.
