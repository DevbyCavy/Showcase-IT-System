...
$statsStmt = $conn->prepare("
    SELECT
        COUNT(*) AS total,
        SUM(status IN ('Assigned', 'Revision Requested')) AS open_jobs,
...
$statsStmt->execute();
$jobStats = $statsStmt->get_result()->fetch_assoc();
$statsStmt->close();
...
/* ---------- My open design jobs (awaiting work or resubmission) ---------- */
$myOpenJobsStmt = $conn->prepare("
    SELECT design_job_id, job_number, title, design_type, deadline, status
    FROM design_jobs
    WHERE designer_id = ? AND status IN ('Assigned', 'Revision Requested')
    ORDER BY deadline ASC
    LIMIT 6
");
$myOpenJobsStmt->bind_param("i", $loggedUserId);
$myOpenJobsStmt->execute();
$myOpenJobs = [];
$mojRes = $myOpenJobsStmt->get_result();
while ($row = $mojRes->fetch_assoc()) $myOpenJobs[] = $row;
...
<div class="dash-grid">
    ...
    <div class="dash-col-main">
        ...
        <div class="dash-card">
            <div class="dash-card-head">
                <h5>My Design Jobs</h5>
                <a class="see-all" href="myDesignJobs.php">See All</a>
            </div>

            <?php if (empty($myOpenJobs)): ?>
                <div class="req-empty">No open design jobs right now.</div>
            <?php else: ?>
                <?php foreach ($myOpenJobs as $job): ?>
                    <a href="myDesignJobs.php" class="req-row" style="text-decoration:none; color:inherit;">
                        <div class="req-icon"><i class="fas fa-pen-ruler"></i></div>
                        <div class="req-body">
                            <div class="req-type"><?= htmlspecialchars($job['job_number']) ?> &middot; <?= htmlspecialchars($job['design_type']) ?></div>
                            <div class="req-title"><?= htmlspecialchars($job['title']) ?></div>
                        </div>
                        <div class="req-meta">
                            <i class="far fa-clock"></i> <?= date('d M Y', strtotime($job['deadline'])) ?>
                        </div>
                    </a>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <div class="dash-col-side">
        ...
        <div class="profile-card">
            ...
            <div class="profile-stats">
                <div class="stat-tile"><strong><?= (int)$jobStats['total'] ?></strong><span>Total Jobs</span></div>
                <div class="stat-tile"><strong><?= (int)$jobStats['open_jobs'] ?></strong><span>Open</span></div>
                <div class="stat-tile"><strong><?= (int)$jobStats['approved'] ?></strong><span>Approved</span></div>
            </div>
        </div>
